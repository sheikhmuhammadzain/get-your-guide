...
import { CART_STORAGE_KEY, type CartItem, sanitizeCartItems } from "@/modules/commerce/cart";
...
export function useCartState() {
  ...
  const setAndPersist = useCallback((updater: CartItem[] | ((prev: CartItem[]) => CartItem[])) => {
    setItems((prev) => {
      ...
      setAndPersist((prev) => {
        ...
        return prev.map((item) =>
          item.productId === productId
            ? { ...item, quantity: Math.max(1, Math.min(10, item.quantity + safeQuantity)) }
            : item,
        );
      });
    },
